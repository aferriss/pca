...
static void onTrackbar(int pos, void* ptr)
{
    ...
    string saveString ="savedImageB";
    string fileType = ".jpg";
    saveString+=std::to_string(ii);
    saveString+=fileType;
    imshow(p->winName, reconstruction);
    imwrite(saveString, reconstruction);
    ii++;
...
int main(int argc, char** argv)
{
    ...
    // perform PCA
    PCA pca(data, cv::Mat(), PCA::DATA_AS_ROW, 1.0); // trackbar is initially set here, also this is a common value for retainedVariance
    ...
    //imshow("pc1", norm_0_255(pca.eigenvectors.row(0)).reshape(1, images[0].rows));
    //imshow("pc2", norm_0_255(eigenvectors.row(100)).reshape(images[0].channels(), images[0].rows));
    ...
    for(int i = 0; i<eigenvectors.rows; i++){
        ...
        string fname = "savedYale/savedImage";
        fname.append(buffer);
        string ext = ".jpg";
        fname += std::to_string(i);
      
        fname += ext;
        imwrite(fname, norm_0_255(eigenvectors.row(i)).reshape(images[0].channels(), images[0].rows));
        }
    }
    ...
    if(key == 'q'){
        ...
        imwrite("savedImage.jpg", reconstruction);
    }
